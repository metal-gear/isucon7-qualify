...
class App < Sinatra::Base
  ...
  get '/message' do
    ...
    statement = db.prepare('SELECT message.*, user.name, user.display_name, user.avatar_icon FROM message JOIN user ON message.user_id = user.id WHERE message.id > ? AND message.channel_id = ? ORDER BY message.id DESC LIMIT 100')
